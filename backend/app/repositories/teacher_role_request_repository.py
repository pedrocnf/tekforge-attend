...
class TeacherRoleRequestRepository:
    ...
    def get_pending_for_user(self, user_id: str) -> dict[str, Any] | None:
        docs = list(self.collection.where("user_id", "==", user_id).where("status", "==", "pending").limit(1).stream())
        if not docs:
            return None
        data = docs[0].to_dict(); data["id"] = docs[0].id
        return data
    ...
    def create(self, user: dict[str, Any], justification: str) -> dict[str, Any]:
        ...
        record = {
            "user_id": user["id"], "username": user["username"], "full_name": user["full_name"], "email": user["email"],
            "status": "pending", "justification": justification, "requested_at": now, "reviewed_at": None, "reviewed_by": None,
        }
        doc_ref.set(record); record["id"] = doc_ref.id
        return record

    def list_pending(self) -> list[dict[str, Any]]:
        out = []
        for doc in self.collection.where("status", "==", "pending").stream():
