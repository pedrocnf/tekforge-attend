...
class AcademicService:
    def __init__(
        self,
        discipline_repo: DisciplineRepository | None = None,
        class_repo: ClassRepository | None = None,
        enrollment_repo: EnrollmentRepository | None = None,
        user_repo: UserRepository | None = None,
    ) -> None:
        self.discipline_repo = discipline_repo or DisciplineRepository()
    ...
    def create_discipline(self, payload: DisciplineCreate) -> dict:
        ...
        return self.discipline_repo.create(
            {
        ...
        return self.class_repo.create(
            {
                ...
                "shift": payload.shift.strip() if payload.shift else None,
            }
        )
    ...
    def create_enrollment(self, payload: EnrollmentCreate) -> dict:
        ...
        class_record = self.class_repo.get_by_id(payload.class_id)
        if not class_record:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Class not found")

        discipline = self.discipline_repo.get_by_id(payload.discipline_id)
        if not discipline:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Discipline not found")

        existing = self.enrollment_repo.find_active(
